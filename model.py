...
with open('training_data/driving_log.csv', 'r') as f:
    ...
    for row in reader:
        ...
        path1 = 'training_data/IMG/' + img_name1 
        path2 = 'training_data/IMG/' + img_name2 
        path3 = 'training_data/IMG/' + img_name3 
...
augmented_images, augmented_s_angles= [],[]

for aug_img,aug_sa in zip(camera_images,steering_angles):
    augmented_images.append(aug_img)
    augmented_s_angles.append(aug_sa)
    
    #Flipping the image
    augmented_images.append(cv2.flip(aug_img,1))
    
    #Reversing the steering angle
    augmented_s_angles.append(aug_sa*-1.0)
  
X_train, y_train = np.array(augmented_images), np.array(augmented_s_angles)
